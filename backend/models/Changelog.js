...
const Changelog = sequelize.define('Changelog', {
  ...
  explanation: { type: DataTypes.STRING(255), allowNull: true },
  release_date: { type: DataTypes.DATE, allowNull: false, defaultValue: DataTypes.NOW },
  content: { type: DataTypes.TEXT('long'), allowNull: true, comment: 'JSON string of sections and items' },
  created_at: { type: DataTypes.DATE, defaultValue: DataTypes.NOW },
  updated_at: { type: DataTypes.DATE, defaultValue: DataTypes.NOW }
}, {
  tableName: 'changelogs',
  timestamps: false,
  hooks: { beforeUpdate: (changelog) => { changelog.updated_at = new Date(); } },
  indexes: [
    { fields: ['project_id'] },
    { fields: ['release_date'] }
  ]
